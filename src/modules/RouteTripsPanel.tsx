...
type Props = {
    routeId: string | null;
    trips: TripSummary[];
    vehicles: VehiclePosition[];        // добавили
    selectedTripId: string | null;
    onSelectTrip: (tripId: string | null) => void;
    onClearRoute: () => void;
};

const RouteTripsPanel: React.FC<Props> = ({
                                              routeId, trips, vehicles, selectedTripId, onSelectTrip, onClearRoute,
                                          }) => {
    ...
    return (
        <div style={{
            ...
            <div style={{ marginBottom: 8 }}>
                <button
                    ...
                    style={{
                        padding: "4px 8px",
                        borderRadius: 6,
                        border: selectedTripId === null ? "2px solid #333" : "1px solid #ddd",
                        background: selectedTripId === null ? "#f3f3f3" : "white",
                        cursor: "pointer"
                    }}
            ...
            <div style={{ marginBottom: 8 }}>
                <div style={{ fontWeight: 600, marginBottom: 6 }}>Рейсы с автобусами ({tripsWithBus.length})</div>
                    ...
                    {tripsWithBus.map(t => (
                        <button
                            ...
                            style={{
                                ...
                                border: selectedTripId === t.tripId ? "2px solid #2c7" : "1px solid #bde5bd",
                                background: selectedTripId === t.tripId ? "#eaffea" : "#f3fff3", cursor: "pointer"
                            }}
                            title="Показать автобус этого рейса"
                        >
                            🚌 <b>{t.tripId}</b>
                            <div style={{ fontSize: 12, opacity: 0.8 }}>dir: {t.directionId} · shape: {t.shapeId}</div>
                        </button>
                    ))}
                    {tripsWithBus.length === 0 && <div style={{ opacity: 0.7 }}>Пока нет активных автобусов</div>}
                </div>
            </div>

            {/* Все рейсы */}
            <div>
                <div style={{ fontWeight: 600, marginBottom: 6 }}>Все рейсы</div>
                <div style={{ display: "grid", gap: 6 }}>
                    {trips.map(t => (
                        <button
                            key={t.tripId}
                            ...
                            style={{
                                textAlign: "left", padding: "6px 8px", borderRadius: 6,
                                border: selectedTripId === t.tripId ? "2px solid #333" : "1px solid #ddd",
                                background: selectedTripId === t.tripId ? "#f3f3f3" : "white", cursor: "pointer"
                            }}
                        >
                            <b>{t.tripId}</b>
                            <div style={{ fontSize: 12, opacity: 0.8 }}>dir: {t.directionId} · shape: {t.shapeId}</div>
